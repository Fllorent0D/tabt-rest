...
import { DivisionDashboardDTOV1, DivisionMemberDashboardDTOV1 } from '../dto/division-dashboard.dto';
...
export class DivisionDashboardService implements DashboardServiceInterface<DivisionDashboardDTOV1> {
  ...
  async getDashboard(divisionId: number): Promise<DivisionDashboardDTOV1> {
    ...
    if (!division.payload) {
      return {
        division: new ResponseDTO(RESPONSE_STATUS.ERROR, undefined, 'No division found for given id'),
        ranking: new ResponseDTO(RESPONSE_STATUS.ERROR, undefined, 'No division found for given id'),
        playersStats: new ResponseDTO(RESPONSE_STATUS.ERROR, undefined, 'No division found for given id'),
      };
    }
  ...
  private async getDivision(divisionId: number): Promise<ResponseDTO<DivisionEntry>> {
    try {
      const division = await this.divisionService.getDivisionsById(divisionId, { ShowDivisionName: 'yes' });
      if(!division) {
        return new ResponseDTO(RESPONSE_STATUS.ERROR, undefined, 'No division found for given id');
      }
  ...
  private async getRanking(divisionId: number): Promise<ResponseDTO<RankingEntry[]>> {
    try {
      const ranking = await this.divisionRankingService.getDivisionRanking({ DivisionId: divisionId });
  ...
  async getPlayersStats(divisionId: number): Promise<ResponseDTO<DivisionMemberDashboardDTOV1[]>> {
    try {
      const results = await this.matchesMembersRankerService.getMembersRankingFromDivision(divisionId);
      // map the results to the DTO
      const playersStats = results.map(DivisionMemberDashboardDTOV1Mapper.mapMemberResults);
