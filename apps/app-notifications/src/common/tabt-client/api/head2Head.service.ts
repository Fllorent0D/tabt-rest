...
import { Head2HeadData } from '../model/head2HeadData';
...
export class Head2HeadService {
   ...
   * @param opponentUniqueIndex
   * @param observe set whether or not to return the data Observable as the body, response or events. defaults to returning the body.
   * @param reportProgress flag to report request and response progress.
   */
  public findHead2HeadMatches(
    playerUniqueIndex: number,
    opponentUniqueIndex: number,
  ): Observable<AxiosResponse<Head2HeadData>>;
  public findHead2HeadMatches(
    playerUniqueIndex: number,
    opponentUniqueIndex: number,
  ): Observable<any> {
    if (playerUniqueIndex === null || playerUniqueIndex === undefined) {
      throw new Error(
        'Required parameter playerUniqueIndex was null or undefined when calling findHead2HeadMatches.',
      );
    }

    if (opponentUniqueIndex === null || opponentUniqueIndex === undefined) {
      throw new Error(
        'Required parameter opponentUniqueIndex was null or undefined when calling findHead2HeadMatches.',
      );
    }
    ...
    return accessTokenObservable.pipe(
      switchMap((accessToken) => {
        ...
        return this.httpClient.get<Head2HeadData>(
          `${this.basePath}/v1/head2head/${encodeURIComponent(String(playerUniqueIndex))}/${encodeURIComponent(String(opponentUniqueIndex))}`,
          {
