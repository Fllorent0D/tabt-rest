...
const routes = [
    ...
    },
    {
        ...
        handle: {
            ...
            cacheDuration: 3600,
            returnUniqueValue: 'MatchId',
            needCredentials: false
        }
    },
    {
        route: '/matchs/:matchUniqueId',
        method: 'GET',
        handle: {
            operation: 'GetMatches',
            args: [
...
router.delete('/flush', (req, res) => {
    cache.flushCache().then((flushed) => {
        if(flushed){
            commonResponse.noData(res)
        } else {
            commonResponse.handleError(res, 'Flush failed')
        }
    });
});
