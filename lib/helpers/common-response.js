...
const request = require('../tabt/soap-request');
...
const handleRequestWithCache = (req, res, opts) => {
    ...
    getResponseFromCache(opts.operation, args, db)
        .then((result) => {
            ...
            logger.debug(`Soap request from ${opts.operation} on ${db} forwarded for ${(ip) ? ip : 'localhost'}`);
            return request(opts.operation, args, db, ip)
                .then((result) => {
        ...
        .then((result) => {
            if (opts.entitiesKey) {
                if (_.get(result, opts.entitiesKey, []).length === 0) {
                    sendData(res, []);
